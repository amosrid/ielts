...
1. "isInvalid": false
2. "tier1": The student's raw original text.
3. "tier2": Grammatically corrected version (fixing grammatical errors, agreement, punctuation, and tense without changing the student's basic tone).
4. "grammarErrors": Array of objects explaining what was wrong in Tier 1 and why:
   - "error": the mistake in original text
   - "explanation": concise, candid reason in Bahasa Indonesia
   - "fix": how to fix it
5. "tier3": Upgraded version (${isIelts ? 'IELTS Band 7.5 - 8.5 academic lexical precision, formal register, complex subordinate clauses, nominalization' : 'Natural, native-sounding idiomatic English with conversational flow'}).
6. "lexicalUpgrades": Array of objects explaining the upgrades:
...
7. "speakingAnchors": Array of 3-4 powerful phrases from Tier 3 that the student should actively recall and use during their upcoming speaking drill.
...
Return ONLY a valid JSON object (no markdown fences, no backticks, no code blocks):
{
  ...
  "speakingAnchors": ["phrase 1", "phrase 2", "phrase 3"]
            ...
            try {
                const response = await callGeminiAPI(`Analyze and transform this essay: "${rawText}"`, systemPrompt);
                const parsed = extractJsonFromLLM(response);
                if (!parsed || (!parsed.tier2 && !parsed.isInvalid)) throw new Error("Gagal menguraikan analisis 3-tier AI.");
...
Return ONLY a valid JSON object (no markdown fences, no backticks, no code blocks):
{
  ...
  "summary": "concluding feedback in Indonesian"
            ...
            try {
                ...
                // Save to Logbook (Zero-Blob: Structured Text & JSON Metadata only!)
                const sessionRecord = {
                    id: 'synth_' + Date.now(),
                    date: Date.now(),
